...
export function contentPromptForBand(promptBase, request) {
  const builders = {
    ...
    hashtags: 'Create exactly 20 targeted hashtags to maximize reach for this band on Instagram and TikTok. Group by category (genre, location, vibe, cover songs). The location category MUST be based on the city and venues in the band details above — read the city from them (e.g. if Mumbai is mentioned, use tags like #mumbailivemusic, #mumbaiband, #mumbaimusicscene). Include at least 4 location tags. Output ONLY the comma-separated list, no intro text.',
    socialcsv: `Create ${request.count || 5} social media posts and output ONLY CSV with exactly 4 columns per row and NO header row: platform,message,hashtags,suggested_date. One post per row, each field wrapped in double quotes with commas inside fields escaped by doubling quotes. Platforms to use: Facebook, Instagram, LinkedIn, Google. Today's date is ${request.today || 'the current date'}; suggested dates must be the next ${request.count || 5} consecutive calendar days starting from tomorrow. No extra text before or after the CSV.`
  };
...
export function contentPromptForBusiness(promptBase, request) {
  const builders = {
    ...
    hashtags: 'Create 20 targeted hashtags to maximize reach for this business on Instagram and TikTok. Mix of popular and niche tags, grouped by category (service, location, audience, offer). For the location category, derive tags from the business location (above), using the actual city and areas. Output as a comma-separated list.',
    whatsapp: 'Create 5 short WhatsApp broadcast messages (under 100 words each) the business can send to local society/community groups to promote their services. Warm, friendly, local tone with a light touch of Hinglish where natural, each ending with a clear call to action and a [PHONE] placeholder. Do not invent addresses or prices.',
    socialcsv: `Create ${request.count || 5} social media posts and output ONLY CSV with exactly 4 columns per row and NO header row: platform,message,hashtags,suggested_date. One post per row, each field wrapped in double quotes with commas inside fields escaped by doubling quotes. Platforms to use: Facebook, Instagram, LinkedIn, Google. Today's date is ${request.today || 'the current date'}; suggested dates must be the next ${request.count || 5} consecutive calendar days starting from tomorrow. No extra text before or after the CSV.`
  };
  return builders[request.type] || builders.blog;
}
...
export function contentPromptForTeacher(promptBase, request) {
  const builders = {
    ...
    hashtags: 'Create 20 targeted hashtags to maximize reach for this kids guitar channel on Instagram and TikTok. Mix of popular and niche tags, grouped by category (kids music, guitar, parents, Mumbai, learning). For the Mumbai category, use the teacher\'s service area (above): based in Malad, teaches across Mumbai. Output as a comma-separated list.',
    whatsapp: 'Create 5 short WhatsApp broadcast messages (under 100 words each) the guitar teacher can send to local Mumbai society/community groups to attract parents for kids guitar lessons. Warm, friendly, local tone with a light touch of Hinglish where natural, each ending with a clear call to action and a [PHONE] placeholder. Emphasize convenience: based in Malad, teaches across Mumbai.',
    socialcsv: `Create ${request.count || 5} social media posts for the channel and output ONLY CSV with exactly 4 columns per row and NO header row: platform,message,hashtags,suggested_date. One post per row, each field wrapped in double quotes with commas inside fields escaped by doubling quotes. Platforms to use: Facebook, Instagram, LinkedIn, Google. Today's date is ${request.today || 'the current date'}; suggested dates must be the next ${request.count || 5} consecutive calendar days starting from tomorrow. No extra text before or after the CSV.`
  };
